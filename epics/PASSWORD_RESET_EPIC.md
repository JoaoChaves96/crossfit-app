...
- [ ] `POST /api/auth/forgot-password` returns a byte-identical empty 200 for a known address, an
      unknown address, a throttled request, and a failed send
- [ ] A reset token is stored only as a hash — the plaintext exists once, in the email
- [ ] A token expires after 1 hour and works exactly once
- [ ] Unknown, expired and used tokens all produce the same 400
- [ ] A second reset request for the same email inside the throttle window sends no mail
- [ ] `POST /api/auth/reset-password` changes the password and returns a working JWT with correct
      role and gym claims
- [ ] Password validation is identical to `POST /api/auth/register`'s
- [ ] A reset link opened while signed out reaches the reset screen, not `/login`
- [ ] The reset screen shows an expired state on mount, before asking for a password
- [ ] A completed reset signs the user in and routes by role
- [ ] A `MailDeliveryError` never escapes the endpoint
- [ ] Swagger at `/api-docs` documents all three endpoints; frontend types are generated, not written
- [ ] jest green in both projects, `tsc` clean, frontend suites pin the responsive register
- [ ] Live walk-through against the log driver: request → read the link from the log → new password
      → signed in
- [ ] Screenshot review at 1280×832 and 390×844
